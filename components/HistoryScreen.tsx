      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
          ...
          dispatches.map(d => (
            <div 
              key={d.id} 
              className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 hover:border-blue-200 transition-colors active:scale-[0.98]"
              onClick={() => d.status === DispatchStatus.COMPLETED ? onSelect(d.dispatch_id) : onResume(d)}
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{d.dispatch_id}</p>
                  <p className="text-sm font-bold text-slate-900 truncate max-w-[150px]">{d.customer_name}</p>
                </div>
              ...
              <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-50">
                <div className="flex flex-col">
                  <span className="text-[8px] text-slate-400 uppercase font-bold">Total Qty</span>
                  <span className="font-mono font-bold text-slate-700 text-xs">{d.total_qty_cached} NOS</span>
                </div>
                <div className="flex flex-col">
                ...
                {d.status === DispatchStatus.DRAFT && (
                   <div className="ml-auto flex items-center gap-1 text-blue-600 font-bold text-[10px] uppercase">
                    RESUME <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                  </div>
                )}
              </div>
            </div>
