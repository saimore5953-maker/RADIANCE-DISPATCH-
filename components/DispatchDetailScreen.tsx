...
const SHEETS_WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbxDlCXUTKzJMDbp_AQrktUo0cWZiB_jKCrE0FvLDAuG55q26c8PuhwnhRJEsCpS9VyW/exec";
...
const DispatchDetailScreen: React.FC<Props> = ({ 
  ...
  // Calculate summaries and track manual entries for UI tagging
  const summaries: Record<string, ExtendedPartSummary> = scans.reduce((acc, scan) => {
    ...
    // If even one scan for this part is a manual entry, we tag the group
  ...
  // G) SCAN LOG FILTER RULE: Hide manual entries from Scan Log UI
  ...
  const handleExport = async (type: 'PDF' | 'EXCEL') => {
    ...
    setExportError(null);
    try {
      // Export hides the manual tag suffix as it uses the clean PartSummary
      const exportSummaries: PartSummary[] = summaryList.map(s => ({
        part_no: s.part_no,
  ...
  const handleFinalize = async () => {
    ...
    try {
      const updatedDispatch: Dispatch = {
        ...
        total_boxes_cached: scans.length,
        total_qty_cached: summaryList.reduce((a, b) => a + b.total_qty, 0),
        parts_count_cached: summaryList.length,
      };
      ...
      setExportedFile(exports.excel); // Auto-trigger success dialog
    } catch (err: any) {
  ...
  const handleUploadToSheet = async () => {
    ...
    try {
      ...
      /**
       * FIX: Use 'text/plain' as Content-Type to avoid CORS preflight (OPTIONS request).
       * Google Apps Script 'exec' URLs often do not handle preflight correctly,
       * leading to 'Failed to fetch' errors in the browser.
       */
      ...
      // Note: With text/plain, the response might still be subject to CORS on the redirect.
      // If 'Failed to fetch' persists, 'no-cors' might be needed, though it prevents reading status.
  ...
  const handleAddManual = async () => {
    ...
    for (let i = 0; i < boxes; i++) {
      const newScan: ScanRecord = {
        ...
        part_no: manualPartNo.trim().toUpperCase(),
        part_name: manualPartName.trim().toUpperCase(),
        qty_nos: qty,
    ...
    <div className="flex-1 flex flex-col bg-slate-50 relative overflow-hidden">
      ...
      <div className="flex bg-white border-b px-2 shrink-0 overflow-x-auto no-scrollbar">
        <button 
          ...
          className={`px-4 py-4 text-[10px] font-bold uppercase tracking-widest border-b-2 transition-all whitespace-nowrap ${activeTab === 'SUMMARY' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400'}`}
        >
          Packing Summary
        </button>
        <button 
          onClick={() => setActiveTab('LOG')}
          className={`px-4 py-4 text-[10px] font-bold uppercase tracking-widest border-b-2 transition-all whitespace-nowrap ${activeTab === 'LOG' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400'}`}
        >
          Full Scan Log
        </button>
        <button 
          onClick={() => setActiveTab('EDIT')}
          className={`px-4 py-4 text-[10px] font-bold uppercase tracking-widest border-b-2 transition-all whitespace-nowrap ${activeTab === 'EDIT' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400'}`}
        >
          Edit Report
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-32">
        ...
        <div className="bg-slate-900 rounded-2xl p-6 text-white mb-6 shadow-xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 relative z-10 border-b border-white/10 pb-4">
            <div>
               <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest block">Executive</span>
               <span className="text-xs font-bold text-blue-400">{dispatch.operator_id}</span>
            </div>
            <div>
               <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest block">Vehicle</span>
               <span className="text-xs font-bold text-white">{dispatch.vehicle_no}</span>
            </div>
            <div>
               <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest block">Driver</span>
               <span className="text-xs font-bold text-white">{dispatch.driver_name}</span>
            </div>
            <div>
               <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest block">LR No</span>
               <span className="text-xs font-bold text-white">{dispatch.lr_no}</span>
            </div>
          </div>
              ...
              <div key={s.part_no} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex justify-between items-center group">
                <div className="flex-1 overflow-hidden pr-2">
                  ...
                  <p className="text-[10px] text-slate-500 uppercase truncate">
                    {s.part_name} {s.is_manual ? <span className="text-blue-600 font-black">(Manual)</span> : ''}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              {filteredScans.length === 0 ? (
                  <p className="text-center text-slate-400 py-10 italic">No camera scans found.</p>
              ) : filteredScans.map((scan, i) => (
                <div 
                  key={scan.id} 
                  className="bg-white p-3 rounded-xl text-xs border border-slate-100 flex items-center justify-between select-none"
                >
        ...
        {activeTab === 'EDIT' && (
          <div className="space-y-6">
            <div className="space-y-3">
              <h3 className="text-[10px] text-slate-400 font-black uppercase tracking-widest ml-1">Correction Workspace</h3>
              {summaryList.length === 0 ? (
                <p className="text-center text-slate-300 py-6 text-xs italic">No entries to edit.</p>
              ) : summaryList.map(s => (
                <div key={s.part_no} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <p className="font-mono font-bold text-slate-900">{s.part_no}</p>
                      <p className="text-[9px] text-slate-500 uppercase truncate max-w-[140px]">
                        {s.part_name} {s.is_manual ? <span className="text-blue-600 font-black">(Manual)</span> : ''}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-blue-600 font-mono font-bold">{s.total_qty} NOS</p>
                      <p className="text-[9px] text-slate-400 font-bold uppercase">{s.boxes} BOXES</p>
                    </div>
                  </div>
                  <div className="flex gap-2 border-t pt-3">
                    <button 
                      onClick={() => onRemoveOne(s.part_no)}
                      className="flex-1 py-2 bg-amber-50 text-amber-700 text-[10px] font-bold uppercase rounded-lg border border-amber-100 active:scale-95 transition-all"
                    >
                      - 1 Box
                    </button>
                    <button 
                      onClick={() => setShowDeleteAllConfirm(s.part_no)}
                      className="flex-1 py-2 bg-red-50 text-red-700 text-[10px] font-bold uppercase rounded-lg border border-red-100 active:scale-95 transition-all"
                    >
                      Remove All
                    </button>
                  </div>
            ...
            {/* Manual Entry Block */}
            <div ref={manualEntryRef} id="manual-entry-form" className="bg-white p-5 rounded-2xl shadow-sm border-2 border-dashed border-slate-200">
              <div className="flex items-center gap-2 mb-4">
                <div className="bg-blue-600 text-white p-1.5 rounded-lg shrink-0">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                </div>
                <h3 className="font-black text-slate-900 text-xs uppercase tracking-[0.15em]">Manual Correction</h3>
              </div>
              
              <div className="space-y-4">
                <div>
                  <label className="text-[9px] font-black text-slate-800 uppercase tracking-widest block mb-1 ml-1">Part Number</label>
                  <input 
                    type="text"
                    value={manualPartNo}
                    onChange={(e) => setManualPartNo(e.target.value.toUpperCase())}
                    className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-mono text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none caret-slate-900"
                    placeholder="E.G. 3B0005299"
                  />
                </div>
                <div>
                  <label className="text-[9px] font-black text-slate-800 uppercase tracking-widest block mb-1 ml-1">Part Name</label>
                  <input 
                    type="text"
                    value={manualPartName}
                    onChange={(e) => setManualPartName(e.target.value.toUpperCase())}
                    className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none caret-slate-900"
                    placeholder="E.G. DOOR LOCK COMP"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[9px] font-black text-slate-800 uppercase tracking-widest block mb-1 ml-1">No. of Boxes</label>
                    <input 
                      type="number"
                      value={manualBoxes}
                  ...
                  <div>
                    <label className="text-[9px] font-black text-slate-800 uppercase tracking-widest block mb-1 ml-1">Qty Per Box</label>
                    <input 
                      type="number"
                      value={manualQtyPerBox}
                      onChange={(e) => setManualQtyPerBox(e.target.value)}
                      className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-mono text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none caret-slate-900"
                      placeholder="0"
                    />
                  </div>
                ...
                <button 
                  ...
                  className="w-full py-4 bg-slate-900 text-white font-bold rounded-xl active:scale-95 transition-all uppercase tracking-widest text-[10px] mt-2 shadow-lg"
                >
      ...
      {/* PATCHED: FIXED TAB-SPECIFIC BOTTOM BAR SYSTEM */}
      <div className="absolute bottom-0 inset-x-0 p-4 bg-white/80 backdrop-blur-md border-t border-slate-100 z-30 pb-8 flex flex-col gap-3">
        {activeTab === 'SUMMARY' && (
          dispatch.status === DispatchStatus.DRAFT ? (
            <button 
              onClick={handleFinalize}
              disabled={isFinalizing}
              className="w-full py-4 bg-green-600 text-white font-bold rounded-2xl shadow-lg shadow-green-500/20 flex items-center justify-center gap-3 uppercase tracking-widest text-xs active:scale-95 transition-all"
            >
            ...
            <div className="flex gap-3">
               <div className="flex-1 py-4 bg-slate-100 text-slate-400 font-bold rounded-2xl flex items-center justify-center gap-3 uppercase tracking-widest text-xs">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                Finalized
              </div>
              <button 
                onClick={handleUploadToSheet}
                disabled={dispatch.sheets_synced || isUploading}
                className={`flex-1 py-4 font-bold rounded-2xl shadow-lg flex items-center justify-center gap-2 uppercase tracking-widest text-xs active:scale-95 transition-all ${
                  dispatch.sheets_synced ? 'bg-emerald-100 text-emerald-600 opacity-80' : 'bg-blue-600 text-white shadow-blue-500/20'
                }`}
              >
                {isUploading ? 'Uploading...' : dispatch.sheets_synced ? 'Uploaded ✅' : 'Upload to Sheet'}
              </button>
            </div>
          )
        )}
        ...
        {activeTab === 'LOG' && (
          <button 
            ...
            className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-lg shadow-blue-500/20 flex items-center justify-center gap-3 uppercase tracking-widest text-xs active:scale-95 transition-all"
          >
        ...
        {activeTab === 'EDIT' && (
          <button 
            onClick={handleManualEntryScroll}
            className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl shadow-lg shadow-slate-900/20 flex items-center justify-center gap-3 uppercase tracking-widest text-xs active:scale-95 transition-all"
          >
      ...
      {/* FINALIZED ALERT (for completed dispatches via Continue Scanning) */}
      {showFinalizedAlert && (
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-md flex items-center justify-center p-8 animate-in fade-in duration-200">
            <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm text-center shadow-2xl">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-2">Batch Finalized</h3>
                <p className="text-slate-500 text-sm mb-8 leading-relaxed">This dispatch is finalized and cannot be continued. Please start a new batch to continue scanning.</p>
                <button onClick={() => setShowFinalizedAlert(false)} className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl active:scale-95 transition-all uppercase tracking-widest text-xs text-center shadow-xl">OK</button>
            </div>
        </div>
      )}
      ...
      {showDeleteAllConfirm && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-md flex items-center justify-center p-6">
            <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm text-center shadow-2xl">
                ...
                <div className="space-y-3">
                    <button onClick={() => onRemoveAll(showDeleteAllConfirm)} className="w-full py-4 bg-red-600 text-white font-bold rounded-2xl active:scale-95 transition-all uppercase tracking-widest text-xs">REMOVE ALL</button>
                    <button onClick={() => setShowDeleteAllConfirm(null)} className="w-full py-4 text-slate-400 font-bold uppercase tracking-widest text-xs">CANCEL</button>
                </div>
            </div>
        </div>
      )}
      ...
      {exportedFile && (
        <div className="fixed inset-0 z-[110] bg-black/80 flex items-center justify-center p-6 backdrop-blur-sm">
            <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2">Batch Finalized</h3>
                
                <div className="bg-slate-50 p-4 rounded-2xl mb-8 text-left border border-slate-100">
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mb-2">Excel Generated</p>
                ...
                <div className="space-y-3">
                    ...
                    <button 
                        onClick={handleUploadToSheet}
                        disabled={dispatch.sheets_synced || isUploading}
                        className={`w-full py-4 font-bold rounded-2xl shadow-lg flex items-center justify-center gap-2 uppercase tracking-widest text-xs active:scale-95 transition-all ${
                          dispatch.sheets_synced ? 'bg-emerald-100 text-emerald-600 opacity-80' : 'bg-blue-600 text-white shadow-blue-500/20'
                        }`}
                    >
                        {isUploading ? 'Uploading...' : dispatch.sheets_synced ? 'Uploaded ✅' : 'Upload to Sheet'}
                    ...
                    <button 
                        ...
                        className="w-full py-3 text-slate-400 font-bold hover:text-slate-600 uppercase tracking-widest text-[10px]"
                    >
                        DONE
                    </button>
                </div>
        ...
        <div className="fixed inset-0 z-[120] bg-black/90 flex items-center justify-center p-6">
            <div className="bg-white rounded-3xl p-6 w-full max-w-sm text-center shadow-2xl border-4 border-red-500">
                ...
                <button 
                    ...
                    className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl active:scale-95 transition-all uppercase tracking-widest text-xs"
                >
                    ACKNOWLEDGE
                </button>
            </div>
