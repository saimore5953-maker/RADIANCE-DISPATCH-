...
const DispatchDetailScreen: React.FC<Props> = ({ 
  ...
  const handleSendTelegram = async () => {
    ...
    if (!telegramBotToken || !telegramChatId) {
      alert("Telegram Bot Token or Chat ID not configured in Settings.");
      return;
    }

    setIsSendingTelegram(true);
    try {
      const { excel } = await generateExports(dispatch, scans, summaryList);
      const totalBoxes = summaryList.reduce((acc, s) => acc + s.boxes, 0);
