...
const DispatchDetailScreen: React.FC<Props> = ({ 
  ...
  const handleSendTelegram = async () => {
    ...
    if (!settings.telegramBotWebhookUrl && settings.telegramBotToken && settings.telegramChatId) {
      if (!confirm("Webhook URL not set. Invoice collection will not be available. Send anyway?")) {
        return;
      }
    }
