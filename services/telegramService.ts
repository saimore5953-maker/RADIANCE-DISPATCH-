...
export const telegramService = {
  async sendDispatchToTelegram(dispatch: Dispatch, summary: PartSummary[], scans: any[], settings: AppSettings) {
    ...
    const payload = {
      ...
      gas_url: settings.webhookUrl // ADDED THIS
    };
    ...
    if (telegramBotWebhookUrl && telegramBotWebhookUrl.trim() !== '') {
      try {
        ...
        // Force relative path for any URL containing 'api/webhook' or pointing to the current origin
        if (targetUrl.includes('/api/webhook') || targetUrl.startsWith(window.location.origin)) {
        ...
        alert(`❌ Connection Failed: ${error.message}\n\nCheck "View Logs" in Settings for details.`);
        throw error;
      }
    }
